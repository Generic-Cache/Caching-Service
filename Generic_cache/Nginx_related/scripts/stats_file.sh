...
# Get log file name
echo -n "The log file which needs to be analysed: "
read FILE

#necessary variables

miss="$(grep -c "ucs=\"MISS\"" $FILE)";
hit="$(grep -c "ucs=\"HIT\"" $FILE)";
stale="$(grep -c "ucs=\"STALE\"" $FILE)";
bypass="$(grep -c "ucs=\"BYPASS\"" $FILE)";
expired="$(grep -c "ucs=\"EXPIRED\"" $FILE)";
updating="$(grep -c "ucs=\"UPDATING\"" $FILE)";
revalidated="$(grep -c "ucs=\"REVALIDATED\"" $FILE)";
total="$(grep -c "rid=" $FILE)";
